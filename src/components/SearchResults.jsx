      ...
      {products && products.map((product, index) => {
        return (
          <Link key={index} to={`/product/${product.id}`}>
            <div className='h-[250px] grid grid-cols-12 rounded
            mt-1 mb-1'>
              <div className='col-span-2 p-4 bg-gray-200'>
                <img className='m-auto' src={product.image_small} />
              </div>
              <div className='col-span-10 bg-gray-50 border-b-gray-100 hover:bg-gray-100'>
                <div className='font-medium text-black p-2'>
                  <ProductsDetails
                    product={product}
                    ratings={true}
                  />
                  <div className='text-xl xl:text-2xl pt-1'>{EURO_CURRENCY.format(product.price)}</div>
                </div>
              </div>
            </div>
          </Link>
